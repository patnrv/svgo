...
<!-- Generated by SVGo -->
<svg width="%d%s" height="%d%s"`
	svgns = `
     xmlns="http://www.w3.org/2000/svg" 
     xmlns:xlink="http://www.w3.org/1999/xlink">`
	vbfmt      = `viewBox="%d%s %d%s %d%s %d%s"`
	emptyclose = "/>\n"
)

// New is the SVG constructor, specifying the io.Writer where the generated SVG is written.
func New(w io.Writer, u string) *SVG { return &SVG{w,u} }
...
func (svg *SVG) Start(w int, h int, ns ...string) {
	svg.printf(svginit, w, svg.Unit, h, svg.Unit)
	for _, v := range ns {
		svg.printf("\n     %s", v)
	}
	svg.println(svgns)
...
func (svg *SVG) Roundrect(x int, y int, w int, h int, rx int, ry int, s ...string) {
	svg.printf(`<rect %s rx="%d" ry="%d" %s`, dim(x, y, w, h, svg.Unit), rx, ry, endstyle(s, emptyclose))
}
...
func (svg *SVG) Image(x int, y int, w int, h int, link string, s ...string) {
	svg.printf(`<image %s %s %s`, dim(x, y, w, h, svg.Unit), href(link), endstyle(s, emptyclose))
}
